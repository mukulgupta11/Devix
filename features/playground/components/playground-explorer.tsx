...
function sortItems(items: TemplateItem[]): TemplateItem[] {
  return [...items].sort((a, b) => {
    ...
    // Both folders — sort alphabetically by folder name
    if (aIsFolder && bIsFolder) {
      return (a as TemplateFolder).folderName.localeCompare((b as TemplateFolder).folderName, undefined, { sensitivity: "base" });
    }

    // Both files — sort alphabetically by full filename
    const aName = `${(a as TemplateFile).filename}.${(a as TemplateFile).fileExtension}`;
    const bName = `${(b as TemplateFile).filename}.${(b as TemplateFile).fileExtension}`;
    ...
    return (
      <SidebarMenuItem>
        ...
        >
          ...
          <CollapsibleContent>
            <SidebarMenuSub>
              {sortItems(folder.items).map((childItem, index) => (
                <TemplateNode
                  key={index}
                  item={childItem}
                  onFileSelect={onFileSelect}
