...
import { Search, Copy, Trash2, Download, Check } from "lucide-react";
...
const TerminalComponent = forwardRef<TerminalRef, TerminalProps>(({ 
  ...
  const terminalTheme = (appTheme === "light" ? "light" : "dark") as "dark" | "light";
  ...
  const copyTerminalContent = useCallback(async () => {
    if (term.current) {
      ...
      // If no text is selected, copy the entire buffer
      if (!content) {
  ...
  useEffect(() => {
    if (term.current) {
      term.current.options.theme = terminalThemes[terminalTheme];
    }
  }, [theme]);
